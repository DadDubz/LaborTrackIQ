...
class LoginHelpRequest(Base):
    __tablename__ = "login_help_requests"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    organization_reference: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    email: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
...
class EmployeeProfile(Base):
    ...
    preferred_weekly_hours: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    preferred_shift_notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class IntegrationConnection(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "provider", name="uq_integration_connections_org_provider"),)
